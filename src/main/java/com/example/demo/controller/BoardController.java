...
public class BoardController extends UiUtils {
	...
	//스프링 4.3 버전부터 @GetMapping, @PostMapping, @PutMapping, @PatchMapping, @DeleteMapping 등
	...
	//required 속성의 기본 값은 true, 만약 idx가 파라미터로 전달되지 않으면 에러가 발생. 그러한 이유로 required 속성을 false로 지정
	...
	public String openBoardDetail(@ModelAttribute("params") BoardDTO params, @RequestParam(value="idx", required=false) Long idx, Model model) {
		...
		if (board == null || "Y".equals(board.getDeleteYn())) {
			...
			//없는 게시글이거나, 이미 삭제된 게시글이라는 메시지를 전달하고, 게시글 리스트로 리다이렉
			return "redirect:/board/list.do";
		}
